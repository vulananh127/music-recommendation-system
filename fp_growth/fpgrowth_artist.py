...
FP-Growth pipeline using mlxtend with safety filters to limit combinatorial explosion.

Features added compared to prior script:
- --max-itemset-len: pass to mlxtend.fpgrowth as max_len (limits itemset size found)
- --max-antecedent-len: drop rules whose antecedent length > N
- --min-lift: drop rules whose lift < value
- diagnostics: prints n_transactions, min_count (ceil(min_support*n)), number of frequent itemsets by size
- compatibility: uses Series.items() instead of iteritems()

Usage example:
  python fpgrowth_artist.py \
    --input Playlist_artist.csv \
    --output rules_artist.csv \
    --min-support 0.015 --min-confidence 0.5 \
    --max-itemset-len 3 --max-antecedent-len 2 --min-lift 1.2 \
    --only-consequent-len 1 --map artist_uri_map.csv

python fpgrowth_artist.py -i Playlist_artist.csv -o rules_artist.csv -m artist_uri_map.csv -s 0.015 -c 0.5 --max-itemset-len 3 --max-antecedent-len 2
...
def parse_list_field(s: str, item_kind: str = "artist") -> List[str]:
    """
    Parse a CSV cell containing comma-separated URIs (possibly quoted).
    ...
    # fallback regex to find spotify:artist:... or spotify:track:...
...
def load_uri_name_map(map_path: str) -> Dict[str, str]:
    ...
    elif 'artist_uri' in cols and 'artist_name' in cols:
        uri_col = mdf.columns[cols.index('artist_uri')]
        name_col = mdf.columns[cols.index('artist_name')]
    else:
        if mdf.shape[1] >= 2:
...
def main(args):
    ...
    if 'list_artist_uri' in cols_lower:
        list_col = cols_lower['list_artist_uri']; item_kind = 'artist'
    elif 'list_track_uri' in cols_lower:
        list_col = cols_lower['list_track_uri']; item_kind = 'track'
    else:
        list_col = None; item_kind = 'artist'
        for c in df.columns:
            lc = c.lower()
    ...
    if list_col is None:
        print("Could not find a 'list_artist_uri' or similar column in input CSV.", file=sys.stderr)
        sys.exit(1)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="FP-Growth (mlxtend) with filters to limit explosion")
    parser.add_argument('--input', '-i', required=True, help="Input CSV path (must contain a column like 'list_artist_uri')")
    parser.add_argument('--output', '-o', default='rules_output.csv', help="Output CSV path")
    parser.add_argument('--min-support', '-s', type=float, default=0.01, help="Minimum support (fraction). Default 0.01")
